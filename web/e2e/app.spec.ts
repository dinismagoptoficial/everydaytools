...
test('setup, local tools, upload, conversion, download, deletion and mobile', async ({ page }) => {
  const external: string[] = [], errors: string[] = [];
  page.on('request', r => { if (/^https?:/.test(r.url()) && !r.url().startsWith('http://127.0.0.1:8087')) external.push(r.url()); });
  page.on('pageerror', e => errors.push(e.message));
  await page.goto('/');
  await expect(page.locator('.auth-form')).toBeVisible();
  if (await page.getByRole('heading', { name: 'Bem-vindo ao Everyday Tools' }).isVisible()) {
  ...
  await page.getByLabel('Conteúdo', { exact: true }).fill('Everyday Tools local');
  ...
  const png = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNgYGBgAAAABQABeqhXUAAAAABJRU5ErkJggg==', 'base64');
  ...
  await page.getByRole('dialog').getByRole('button', { name: /Converter imagem/ }).click();
  await page.getByLabel('Formato de saída').selectOption('jpg');
  await page.getByRole('button', { name: 'Processar ficheiro' }).click();
  await expect(page.getByText('Concluído', { exact: true }).first()).toBeVisible({ timeout: 60000 });
  const downloadPromise = page.waitForEvent('download');
  await page.getByRole('link', { name: 'Descarregar', exact: true }).first().click();
  const download = await downloadPromise;
  expect(download.suggestedFilename()).toMatch(/\.jpg$/);
  expect(await download.failure()).toBeNull();
  await page.getByRole('button', { name: 'Eliminar agora: ' + fileName }).click();
  await page.getByRole('dialog').getByRole('button', { name: 'Eliminar agora', exact: true }).click();
  ...
  await page.getByRole('button', { name: 'Ficheiros recentes', exact: true }).click();
  await expect(page.getByRole('heading', { name: 'Ficheiros recentes' })).toBeVisible();
  expect(external).toEqual([]);
  expect(errors).toEqual([]);
});

test('registration and login use separate personal file lists', async ({ page }) => {
  await page.goto('/');
  await expect(page.locator('.auth-form')).toBeVisible();
